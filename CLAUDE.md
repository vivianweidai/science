...
- **Jupyter notebook conventions:**
  ...
  - **Chart styling** — set `facecolor='#f6f8fa'` on both the figure and axes to match the gray output boxes. Use Times New Roman for all chart text (`plt.rcParams['font.family'] = 'serif'`, `plt.rcParams['font.serif'] = ['Times New Roman']`, `plt.rcParams['mathtext.fontset'] = 'dejavuserif'`). Use pastel colours for data (e.g., `#f8d7da`, `#d4edda`, `#cce5ff`, `#fff3cd`, `#e2d9f3`). Remove top/right/left spines, disable left ticks, add light y-grid (`alpha=0.3`). Save as both PNG (300 dpi) and SVG with matching facecolor.
