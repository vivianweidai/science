...
- **Jupyter notebook conventions:**
  ...
  - **Chart styling** — set `facecolor='#f6f8fa'` on both the figure and axes to match the gray output boxes. Use Times New Roman for all chart text (`plt.rcParams['font.family'] = 'serif'`, `plt.rcParams['font.serif'] = ['Times New Roman', 'DejaVu Serif', 'serif']`, `plt.rcParams['mathtext.fontset'] = 'dejavuserif'`). Suppress font warnings for Colab compatibility (`warnings.filterwarnings('ignore', message='.*findfont.*')`). Use soft, muted pastels for all chart colours — the reference palette is: pink `#f8d7da`, green `#d4edda`, blue `#cce5ff`, yellow `#fff3cd`, purple `#e2d9f3`, orange `#fce4b8`. For data line traces use slightly deeper tones like `#d95f5f`. Always keep colours light and airy — never saturated or bold. Remove top/right/left spines, disable left ticks, add light y-grid (`alpha=0.3`). Save as both PNG (300 dpi) and SVG with matching facecolor.
