...
├── olympiads/              # Olympiads page (renders from archives/*.json)
├── research/               # Research projects
│   ├── index.md            # Research landing with tabbed project table
│   └── YYYYMMDD Project/   # Individual projects (see below)
├── content/                # Source of truth for olympiads and textbooks
│   ├── olympiads.yml       # Olympiad events — edit this, then rebuild
│   └── textbooks.yml       # Textbooks and courses — edit this, then rebuild
├── scripts/
│   └── build_listings.py   # content/*.yml → archives/*.json (run after edits)
├── apple/                  # iOS app (read-only consumer of archives/*.json)
└── archives/               # Shared assets
    ├── IMAGES/              # Consolidated images
    ├── olympiads.json       # Generated by build_listings.py — DO NOT EDIT BY HAND
    └── textbooks.json       # Generated by build_listings.py — DO NOT EDIT BY HAND
```

**Olympiads and textbooks workflow:** `content/olympiads.yml` and `content/textbooks.yml` are the single source of truth. After editing either file, run `python scripts/build_listings.py` to regenerate `archives/olympiads.json` and `archives/textbooks.json`, then commit both the YAML and the JSON. The GitHub Actions workflow (`.github/workflows/pages.yml`) validates that the committed JSON matches a fresh build and refuses to deploy if it's stale. Both the webapp (`olympiads/index.md` via client-side JS) and the iOS app (`apple/shared/API/APIClient.swift`) fetch the JSON directly — there is no database, no API, no admin endpoint.
