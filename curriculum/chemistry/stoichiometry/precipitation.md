---
subject: Chemistry
section: Stoichiometry
topic: Reactions
table: Precipitation
order: 3
---

# Precipitation

<table>
<tr><th>Label</th><th>Formula / Concept</th><th>Description</th></tr>
<tr class="highlight"><td>precipitation</td><td>two aqueous solutions</td><td>makes solid precipitate</td></tr>
<tr class="highlight"><td>precipitation</td><td>competing forces</td><td>larger charge less soluble</td></tr>
<tr><td>soluble</td><td>alkali metals</td><td>$Na^{+}Li^{+}K^{+}$</td></tr>
<tr><td>soluble</td><td>ammonium</td><td>$NH_{4}^{+}$</td></tr>
<tr><td>soluble</td><td>halogens sulfate</td><td>$Cl^{-}Br^{-}I^{-}$</td></tr>
<tr><td>soluble</td><td>single charge polyatomics</td><td>$NO_{3}^{-}C_{2}H_{3}O_{2}^{-}ClO_{3}^{-}$</td></tr>
<tr><td>soluble</td><td>sulfate</td><td>$SO_{4}^{2 -}$</td></tr>
<tr><td>insoluble</td><td>single charge metal</td><td>$Ag^{+}Hg_{2}^{2 +}$</td></tr>
<tr><td>insoluble</td><td>double charge metals</td><td>$Ba^{2 +}Ca^{2 +}Pb^{2 +}Mg^{2 +}$</td></tr>
<tr><td>insoluble</td><td>multi charge polyatomics</td><td>$CO_{3}^{2 -}PO_{4}^{3 -}$</td></tr>
</table>
